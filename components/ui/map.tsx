/* biome-ignore-all lint/correctness/useExhaustiveDependencies: Generated mapcn component intentionally controls hook dependencies for MapLibre lifecycle and performance. */
"use client";
...
const CONTROL_BUTTON_TAP = { scale: 0.8 } as const;
const CONTROL_BUTTON_TAP_TRANSITION = {
  duration: 0.12,
  ease: "easeOut",
} as const;

function ControlGroup({ children }: { children: React.ReactNode }) {
  return <div className="flex flex-col overflow-hidden rounded-md bg-primary shadow-sm">{children}</div>;
}
...
function ControlButton({
  ...
  return (
    <motion.button
      ...
      aria-label={label}
      type="button"
      whileTap={disabled ? undefined : CONTROL_BUTTON_TAP}
      transition={CONTROL_BUTTON_TAP_TRANSITION}
      className={cn(
        "flex size-9 items-center justify-center rounded-sm border-foreground bg-primary text-white transition-colors hover:bg-primary/80! dark:hover:bg-accent/40",
      ...
      {showLocate && (
        <ControlGroup>
          <ControlButton onClick={handleLocate} label="Find my location" disabled={waitingForLocation}>
            {waitingForLocation ? <Loader2 className="size-4 animate-spin" /> : <Locate className="size-5" strokeWidth={2.5} />}
          </ControlButton>
        </ControlGroup>
      )}
      {showFullscreen && (
