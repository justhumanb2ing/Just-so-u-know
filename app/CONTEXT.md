...
- `[handle]`: 공개 페이지 렌더링과 페이지 프로필 저장 서버 액션
- `layout.tsx`: 전역 레이아웃
- `changelog/page.tsx`: MDX 릴리즈 노트 렌더링 페이지
...
- 페이지 공개 여부 토글은 `[handle]/actions.ts` 서버 액션에서 인증/입력 검증 후 서비스(`toggleOwnedPageVisibility`)로 위임한다.
- 페이지는 리다이렉트 조건을 빠르게 평가하고 필요 시 즉시 `redirect()`/`notFound()`를 반환한다.
- 공개 페이지 편집은 서버 컴포넌트에서 소유자 여부를 판별한 뒤, 소유자에게만 클라이언트 편집 컴포넌트를 노출한다.
- changelog 페이지는 `service/versioning`에서 검증된 엔트리만 렌더링한다.
